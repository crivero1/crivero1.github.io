        ...
        {listToParse.map((line, index) => (
            <div key={index} className="text-left text-body my-4">
                <div className="flex">
                    ...
                    <p className="pl-3">
                        <a href={line["link"]} className={`italic font-semibold`}>{line["title"]}</a>. Together with {line["authors"]}. In {line["type"]} {line["year"]}. <a href={line["pdf"]} className={`${bold_color}`}>{line["pdf"] != "" ? '(pdf)' : ''}</a>
                    </p>
                </div>
            </div>
        ))}
